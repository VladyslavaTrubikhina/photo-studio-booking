...
The "default" visitor of the website. All users can see the reservable items. However, in order to make a reservation
the user should be logged into the system.

## Admin

An Administrator manages the reservable items. An admin can create, update and remove the items but cannot reserve
items. In order to reserve an item the administrator must also be a regular user.
